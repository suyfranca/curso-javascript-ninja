(function(){
  ...
  */
  ...
  let cpfs = []
  cpfs.push(cpf1, cpf2, cpf3, cpf4);

  // cpf1 = cpf1.replace( /(\d{3})(\d{3})(\d{3})(\d{2})/g , ' $1.$2.$3-$4 ');
  // cpf2 = cpf2.replace( /(\d{3})(\d{3})(\d{3})(\d{2})/g , ' $1.$2.$3-$4 ');
  // cpf3 = cpf3.replace( /(\d{3})(\d{3})(\d{3})(\d{2})/g , ' $1.$2.$3-$4 ');
  // cpf4 = cpf4.replace( /(\d{3})(\d{3})(\d{3})(\d{2})/g , ' $1.$2.$3-$4 ');
  ...
  */
  ...
  let phrase = 'Os meses de janeiro, junho e julho começam com a letra j.';

  let regex = phrase.match( /ju[nl]ho/g );
  console.log( regex );
  ...
  */
  ...
  let code = '<div><section><blockquote>Texto <img /></blockquote></section></div>';

  let regexCode = code.match( /<\w{1,}>/g );
  ...
  */
  ...
  code = '<div><ul><li></li><li></li><li><span></span></li></ul></div>';

  regexCode = code.match( /<\w{1,}><\/\w{1,}>/g );
  console.log( regexCode );
  ...
  */
  ...
  let text = '<h1>Titulo da pagina</h1><p>Este e um paragrafo</p><footer>Rodape</footer>';

  console.log( text.replace( /<(\w+)>([^<]+)<\/\w+>/g , `<$1> O texto dentro da tag "$1" é "$2" </$1> \n` ) );
